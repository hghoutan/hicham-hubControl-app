...
class DayColumn extends StatefulWidget {
  ...
  late  Time? oldTime ;
  DayColumn({required this.times,super.key, this.oldTime});
...
@override
  ...
  Widget build(BuildContext context) {
    ...
    if (widget.oldTime != null) {
      ...
      children.add(
          Container(
        height: (widget.times[0].stTime! / 1440) * Dimension.getScreenHeight() * .7,
      ...
      for (var i=0;i<widget.times.length;i++){
        print("${widget.times[i].stTime! } - ${widget.times[i].comfortSetting!}");
        ...
        if (widget.times[i].stTime == widget.times[0].stTime) {
          children.add(
              Container(
                height: ((widget.times[i + 1].stTime! - widget.times[i].stTime!) / 1440) * Dimension.getScreenHeight() * .7,
                width: Dimension.getScreenWidth(),
                decoration: BoxDecoration(
          ...
          children.add(
              Container(
                height:
                ((widget.times[i + 1].stTime! - widget.times[i].stTime!) / 1440) *
                    Dimension.getScreenHeight() * .7,
                width:Dimension.getScreenWidth(),
                color:color,
              )
          );
    ...
    return  Container(
      ...
      width: Dimension.getScreenWidth() * 4/100,
      decoration:  BoxDecoration(
